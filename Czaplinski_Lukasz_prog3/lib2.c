...
#define PAGE_OCCUPIED_INDEX(page_ptr, ptr) (((char*)(ptr) - (char*)(page_ptr)) / PAGE_GRAIN)
...
{
  for (unsigned int i = 0; i < NODE_MAX_NUMBER; i++) {
    page->occupied_nodes[i] = -1;
  }
...
#define PCON_CHECKINIT(X) { if( ! PCON_IS_INIT(X) ) { pcontrol_init(X); } }
